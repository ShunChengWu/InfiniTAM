    ...
    public:
         static void show(ORUtils::Image<float> *img, cv::Mat **mat) {
            ...
            for (int i = 0; i < img->noDims.y; ++i) {
                for (int j = 0; j < img->noDims.x; ++j) {
                    (*mat)->at<float>(i, j) = img->GetData(MEMORYDEVICE_CPU)[i * img->noDims.x + j];
                }
            }
            double min;
            double max;
        ...
        static void show(ORUtils::Image<Vector3<float>> *img, const std::string &name = "image") {
            ...
            for (size_t c = 0; c < 3; ++c) {
                ...
                for (int i = 0; i < img->noDims.y; ++i) {
                    for (int j = 0; j < img->noDims.x; ++j) {
                        mat.at<float>(i, j) = img->GetData(MEMORYDEVICE_CPU)[i * img->noDims.x + j][c];
                    }
                }
                double min;
                double max;
            ...
            for (int i = 0; i < img->noDims.y; ++i) {
                for (int j = 0; j < img->noDims.x; ++j) {
                    for (size_t c = 0; c < 4; ++c)
                        mat.at<cv::Vec<uchar, 4>>(i, j)[c] = img->GetData(MEMORYDEVICE_CPU)[i * img->noDims.x + j][c];
                }
            }
...
//            cv::convertScaleAbs(mat, mat, 255 / max);
        ...
        static void show(ORUtils::Image<unsigned char> *img, const std::string &name = "image") {
            ...
            for (int i = 0; i < img->noDims.y; ++i) {
                for (int j = 0; j < img->noDims.x; ++j) {
                    for (size_t c = 0; c < 3; ++c)
                        mat.at<cv::Vec<uchar, 4>>(i, j)[c] = img->GetData(MEMORYDEVICE_CPU)[i * img->noDims.x + j];
